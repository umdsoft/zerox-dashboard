...
async function load() {
  ...
  try {
    ...
    rows.value = list.map((it) => {
      // possible backend field names unified
      const debtorName =
        it.debitor_name ||
        [it.d_last_name, it.d_first_name, it.d_middle_name].filter(Boolean).join(' ')
      const debtorUid = it.debitor_uid || it.d_uid || it.debitorUid

      return {
        debitor_name: debtorName || '—',
        debitor_uid: debtorUid,
        amount: it.amount ?? 0,
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-semibold text-slate-800">Muddati o‘tgan (kreditor)</h1>
    </div>
